...
const pkg = (id: string, order = 0, extra: Partial<ServicePackageData> = {}): ServicePackageData => ({
...
const base: ProjectState = { settings, packages: [pkg("a", 0), pkg("b", 1)], milestones: [ms("m")] };
...
describe("project lifecycle", () => {
  it("configures, loads and resets", () => {
    ...
    const loaded = projectReducer(configured, { type: "project/loaded", file: { version: "1.1", exportedAt: "", projectSettings: settings, servicePackages: [pkg("z")], milestones: [] } });
    expect(loaded.packages.map(p => p.id)).toEqual(["z"]);

    expect(projectReducer(loaded, { type: "project/reset" })).toBe(INITIAL_PROJECT_STATE);
  });
});

describe("packages and milestones", () => {
  it("assigns the next order to a new package and replaces an existing one", () => {
    const added = projectReducer(base, { type: "package/saved", pkg: pkg("c", 0) });
    expect(added.packages.find(p => p.id === "c")?.order).toBe(2);

    const updated = projectReducer(added, { type: "package/saved", pkg: pkg("a", 0, { name: "renamed" }) });
    expect(updated.packages.find(p => p.id === "a")?.name).toBe("renamed");
    expect(updated.packages).toHaveLength(3);
  });

  it("deletes by id", () => {
    expect(projectReducer(base, { type: "package/deleted", id: "a" }).packages.map(p => p.id)).toEqual(["b"]);
  ...
  it("moves a single package like a block of one", () => {
    const s = projectReducer(base, { type: "packages/moved", ids: ["b"], direction: "up" });
    expect([...s.packages].sort((x, y) => x.order - y.order).map(p => p.id)).toEqual(["b", "a"]);
  });
});

describe("labels", () => {
  it("accumulates drag deltas on the right label", () => {
    const s1 = projectReducer(base, { type: "label/dragged", item: "package", label: "name", id: "a", delta: { x: 3, y: -2 } });
    expect(s1.packages[0]).toMatchObject({ labelOffsetX: 3, labelOffsetY: -2, dateLabelOffsetX: 0 });
  ...
  it("resets offsets of the selected items only", () => {
    const dirty: ProjectState = { ...base, packages: [pkg("a", 0, { labelOffsetX: 9 }), pkg("b", 1, { labelOffsetX: 9 })] };
    const s = projectReducer(dirty, { type: "items/labelsReset", ids: { packages: ["a"], milestones: [] } });
    expect(s.packages[0].labelOffsetX).toBe(0);
    expect(s.packages[1].labelOffsetX).toBe(9);
  });

  it("migrates legacy label offsets", () => {
    const legacy: ProjectState = { ...base, packages: [pkg("a", 0, { labelOffsetX: 10, labelOffsetY: 0, dateLabelOffsetX: 10, dateLabelOffsetY: 15 })] };
    const s = projectReducer(legacy, { type: "labels/migrated" });
    expect(s.packages[0]).toMatchObject({ labelOffsetX: 0, dateLabelOffsetY: 0 });
  });
});

describe("bulk actions", () => {
  const ids = { packages: ["a"], milestones: ["m"] };

  it("patches shared fields on both kinds and package-only fields on packages", () => {
    const s = projectReducer(base, { type: "items/patched", ids, patch: { color: "#FF0000", showTextInside: true } });
    expect(s.packages[0]).toMatchObject({ color: "#FF0000", showTextInside: true });
    expect(s.packages[1].color).toBe("#000000");
  ...
  it("shifts dates within the project range", () => {
    ...
    expect(s.packages[0]).toMatchObject({ startDate: "2026-03-08", endDate: "2026-04-07" });
    expect(s.milestones[0].date).toBe("2026-06-08");
    expect(s.packages[1].startDate).toBe("2026-03-01");
  });
  ...
  it("deletes the selection", () => {
    ...
    expect(s.packages.map(p => p.id)).toEqual(["b"]);
